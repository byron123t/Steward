...
SAFE_ATTRS = frozenset({'class', 'type', 'datetime', 'readonly', 'src', 'id', 'value', 'label', 'title', 'alt', 'href', 'checked', 'selected', 'nohref', 'name', 'role', 'aria-label', 'placeholder', 'data-placeholder', 'data-action', 'data-event', 'hidden', 'aria-hidden', 'disabled', 'aria-disabled', 'aria-checked', 'tabindex', 'data-qa', 'data-gaid', 'data-pageid', 'x-ref', 'x-effect', 'x-on:click', 'target'})
...
def remove_aria_hidden(element):
    parser = etree.HTMLParser()
    tree = etree.fromstring(element, parser)
    for child in tree.getchildren():
        for grandchild in child.getchildren():
            for attrib_name, attrib_value in grandchild.attrib.items():
                if attrib_name == 'aria-hidden' and attrib_value == 'true':
                    grandchild.getparent().remove(grandchild)
    modified_html = etree.tostring(grandchild, encoding='unicode')
    return modified_html
